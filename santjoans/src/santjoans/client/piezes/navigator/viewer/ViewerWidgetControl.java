...
public class ViewerWidgetControl extends ViewerWidget implements IConfiguration, ICanvasEventEnabledListener {
	...
	private int pixelsToMillimetersX(int pixel) {
		return ((pixel * controllerViewer.getContext().getZoomMode().getMillimetersWidth()) / Util.getCurrentScreenType().getCanvasX());
	}
	
	private int pixelsToMillimetersY(int pixel) {
		return ((pixel * controllerViewer.getContext().getZoomMode().getMillimetersHeight()) / Util.getCurrentScreenType().getCanvasY());
	}
